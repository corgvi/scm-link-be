...
public class InventoryLocationDetailService extends BaseServiceImpl<InventoryLocationDetailRequest, InventoryLocationDetailRequest, InventoryLocationDetailResponse, InventoryLocationDetail, String> {
    ...
    public InventoryLocationDetailResponse createOrUpdate(InventoryLocationDetailRequest dto, String warehouseId, String productId) {
        InventoryLocationDetail inventoryLocationDetail = inventoryLocationDetailRepository.findBySameBatch(productId, warehouseId, dto.getBatchNumber()).orElse(null);
        if (inventoryLocationDetail == null) {
            WarehouseLocation location = warehouseLocationRepository.findByIdAndWarehouse_Id(dto.getWarehouseLocationId(), warehouseId).orElseThrow(() -> new AppException(ErrorCode.WAREHOUSE_LOCATION_NOT_IN_WAREHOUSE));
            inventoryLocationDetail = inventoryLocationDetailMapper.toEntity(dto);
            inventoryLocationDetail.setWarehouseLocation(location);
            inventoryLocationDetail.setInventoryLevel(inventoryLevelRepository.findByWarehouse_IdAndProduct_Id(warehouseId, productId).orElseThrow(() -> new AppException(ErrorCode.INVENTORY_LEVEL_NOTFOUND)));
        } else {
        ...
        ProductResponse productResponse = productMapper.toProductResponse(inventoryLocationDetail.getInventoryLevel().getProduct());
        log.info("Product response in inventory location detail: {}", productResponse);

        InventoryLocationDetailResponse inventoryLocationDetailResponse = inventoryLocationDetailMapper.toDTO(inventoryLocationDetail);
